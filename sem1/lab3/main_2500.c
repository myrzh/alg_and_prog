...
void subtract(const BigInt* a, const BigInt* b, BigInt* result) {
    ...
    fromInt(&tempNull, 0);
    tempNull.length = 1;
    if (compare(a, &tempNull) == 0 && compare(b, &tempNull) == 0) {
        result->sign = 1;
        memcpy(result, &tempNull, sizeof(BigInt));
        return;
    }
...
void sumRange(const BigInt* start, const BigInt* end, BigInt* result) {
    ...
    if (compare(start, end) == 0 && start->sign != end->sign)
    {
        initialize(&current);
        current.length = 1;
        memcpy(result, &current, sizeof(BigInt));
        return;
    }

    // Решение при разных знаках
    if (start->sign != end->sign)
    {
        BigInt newStartNull, changedOldStart, changedOldEnd, result1, result2;
        initialize(&newStartNull);
        newStartNull.length = 1;
        initialize(&changedOldStart);
        memcpy(&changedOldStart.digits, &start->digits, sizeof(BigInt));
        changedOldStart.sign = 1;
        initialize(&changedOldEnd);
        memcpy(&changedOldEnd.digits, &end->digits, sizeof(BigInt));
        changedOldEnd.sign = 1;
        sumRange(&newStartNull, &changedOldStart, &result1);
        sumRange(&newStartNull, &changedOldEnd, &result2);
        subtract(&result2, &result1, result);
        return;
    }
...
void evaluateExpression(const char* expression) {
    BigInt operandStack[5];
    char operatorStack[MAX_DIGITS / 2];
    int operandTop = -1;
    int operatorTop = -1;
    int lastOperand = -1;
    ...
    for (int i = 0; i < length; i++) {
        ...
        if (currentChar == ' ') {
            ...
            if (lastOperand == 0) // Проверка на некорректный ввод факториала
                exit(0);
            BigInt result;
            BigInt temp;
            fromInt(&temp, 120); // Порог расчёта факториала
            if (compare(&operand, &temp) == -1)
...
int main() {
    ...
    if (strcmp(input, "") == 0) // Проверка на пустую строку
        return 0;

    // Проверяем каждый символ в строке input на корректность
    int operandsCount = 0;
    for (int i = 0; i < strlen(input); i++) {
        ...
        // Проверяем разделение: на выражение и на сумму от и до
        if (!isdigit(currentChar) && currentChar != ' ')
            operandsCount++;
        if (isdigit(currentChar) && operandsCount == 0)
            operandsCount = 2;
    }
